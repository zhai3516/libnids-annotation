...
// 指向最新的 pcap 返回的链路层数据帧头部的指针
struct pcap_pkthdr * nids_last_pcap_header = NULL;

// 指向最新的 pcap 返回的数据帧数据部分的指针
u_char *nids_last_pcap_data = NULL;

// 链路层数据帧有效数据的偏移量，可以理解为头部长度，用于确定数据部分起始指针指向的地址
...
// 并没有做什么，是 nids_params 过滤ip分组的默认方法，本质上不过滤任何分组
 ...
 */
// 依次调用注册的 ip 碎片回调函数
// 注册回调函数长度函数 ：nids_register_ip_frag
...
#define ETHERTYPE_IP 0x0800 // ip 协议对应 0x0800，其他还有 icmp 等

/*
 这个函数是注册都 pcap 中的 handler 函数，每当 pcap 捕获到链路层帧，都会调用这个回调函数
 struct pcap_pkthdr *hdr 是 pcap 自定义的链路层数据帧头结构指针
 u_char * data 是 pcap 捕获的数据帧的指针。
 其中 struct pcap_pkthdr 架构比较简单，如下：
 struct pcap_pkthd
 {
 struct timeval ts;   // 时间戳结构，包含 s 和 ms 部分
 bpf_u_int32 caplen; // 整个数据帧去除头、尾后的有效数据部分的长度
 bpf_u_int32 len;  // 整个数据帧的长度
 }
...
void nids_pcap_handler(u_char * par, struct pcap_pkthdr *hdr, u_char * data)
{
    ...
    (void)par; /* warnings... */
    switch (linktype) {
            // 设置nids_linkoffset， 一般（以太网）DLT_EN10MB nids_linkoffset为14
        case DLT_EN10MB:
            if (hdr->caplen < 14)
                return;
...
#error DLT_PRISM_HEADER is defined, but DLT_IEEE802_11 is not ???
     ...
     */
    // 计算出指向ip header的指针位置 data_aligned = data + linkoffset
#ifdef LBL_ALIGN
...
/*
 根据传入的链路层帧（Frame），获取网络层ip分组（Packet）
 */
static void gen_ip_frag_proc(u_char * data, int len)
{
    struct proc_node *i;
    struct ip *iph = (struct ip *) data; // 解析 ip header
    int need_free = 0;
    int skblen;
    void (*glibc_syslog_h_workaround)(int, int, struct ip *, void*)=
    nids_params.syslog; // 一个函数指针，glib 使用 libnids 的 log 输出函数
    ...
    // 根据ip header 判断是否是有效地 ip 分组
    // 条件1 len < (int)sizeof(struct ip) ：传入的 ip 分组大小须大于ip header的大小，意味着，必须包含一个完成的头部
    // 条件2 iph->ip_hl < 5 ：头部长度要大于5
    // 条件3 iph->ip_v != 4 ：只接受 ipv4
    // 条件4 ip_fast_csum((unsigned char *) iph, iph->ip_hl) != 0 ： TODO ：？？？？
    // 条件5 len < ntohs(iph->ip_len) ： 传输的pcap给出的数据长度需要大于从 ip header 中解析出的报文长度，意味着必须是完整的分组
    // 条件6 ntohs(iph->ip_len) < iph->ip_hl << 2 ： TODO ：？？？？
    ...
    switch (ip_defrag_stub((struct ip *) data, &iph)) {
        case IPF_ISF: // error
            return;
        case IPF_NOTF: // 单独分片就是一个数据，没有分片
            need_free = 0;
            iph = (struct ip *) data;
            break;
        case IPF_NEW: // 整理好新的分片
            need_free = 1;
            break;
        default:;
    }
...
// 初始化 libnids，包括各种参数的读入，以及一些数据结构的初始化
int nids_init()
{
...
// 对应 pcap 的 pcap_get_selectable_fd
int nids_getfd()
{
    if (!desc) {
        strcpy(nids_errbuf, "Libnids not initialized");
